  ...
  return (
    <div className="h-full overflow-y-auto no-scrollbar px-4 pb-6 pt-5">
      ...
      <p className="mb-5 mt-1 text-sm text-muted-foreground">
        Ancient sciences, decoded by AI for you.
      </p>
  ...
  return (
    <div>
      <p className="mb-5 text-sm text-muted-foreground">
        Your thumb impression unlocks the ancient leaf manuscript said to be
        written by the sage Agastya.
      </p>
      ...
      {state !== "done" ? (
        ...
        >
          ...
          >
            <Fingerprint
              className={`h-14 w-14 ${state === "scanning" ? "text-saffron" : "text-gold"}`}
            />
          </div>
          <span className="text-base font-semibold text-foreground">
            {state === "scanning"
              ? "Reading thumb impression…"
              : "Scan Thumbprint"}
          </span>
          {state === "scanning" && (
        ...
        <div className="rounded-3xl border border-gold/30 bg-gradient-to-b from-surface to-surface-2 p-5">
          ...
          <p className="mb-3 text-[15px] leading-relaxed text-foreground">
            "He who is born in the year {yr}, named {fn}, carries the mark of
            Jupiter upon the second thumb ridge. In youth he wanders; in
            maturity he commands respect through knowledge."
          </p>
          <div className="space-y-2 text-sm text-muted-foreground">
            <p>
              📜 <b className="text-foreground">Past Kanda:</b> Karmic debt of
              speech, now cleared.
            </p>
            <p>
              🪔 <b className="text-foreground">Present Kanda:</b> A door of
              opportunity opens within 90 days.
            </p>
            <p>
              ✨ <b className="text-foreground">Remedy:</b> Light a ghee lamp on
              Thursdays and feed Brahmins.
            </p>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-muted-foreground">
        Practical, low-cost remedies (सरल उपाय) tuned to the doshas detected in
        your chart (ruling planet: {ruler}).
      </p>
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted-foreground">
        Predictions for the world, drawn from current planetary transits.
      </p>
      {feed.map((f, i) => (
        <article
          key={i}
    ...
    <div>
      <div className="mb-4 rounded-2xl border border-gold/30 bg-saffron/10 p-4">
        <p className="text-sm font-medium text-foreground">
          Don't know your birth time? Ask your question now.
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          A Prashna chart is cast for the exact moment you ask.
        </p>
      </div>
        ...
        <div className="mt-5 rounded-2xl border border-border bg-surface p-4">
          ...
          <div className="mx-auto mb-4 grid aspect-square w-48 grid-cols-3 grid-rows-3 gap-px overflow-hidden rounded-lg border border-gold/40 bg-gold/30">
            {Array.from({ length: 9 }).map((_, i) => (
              ...
              >
                {i === 0 ? (
                  <span className="font-bold text-gold">Asc</span>
                ) : i === 4 ? (
                  "♄"
                ) : i === 2 ? (
                  "☉"
          ...
          <div className="space-y-1.5 text-sm">
            <p className="text-muted-foreground">
              Ascendant: <b className="text-foreground">{result.asc}</b>
            </p>
            <p className="text-muted-foreground">
              Moon sign: <b className="text-foreground">{result.moon}</b>
            </p>
            <p className="mt-3 rounded-xl bg-saffron/10 p-3 text-[15px] leading-relaxed text-foreground">
              {result.verdict}
            </p>
          </div>
        </div>
...
function MedicalView() {
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-muted-foreground">
        Each planet governs a region of the body. Here is your personalized
        Astro-Wellness map.
      </p>
      <div className="space-y-3">
        {map.map((m, i) => (
          ...
          >
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-saffron/15 text-gold">
              {m.icon}
            </div>
